...
The executed fixed-$A_{\rm Planck}=1$ conditional surface contains 154 KGB
points. It combines official Planck ordinates with exact-background Pantheon+
relative distances, DESI DR2 BAO, and cosmic chronometers, using the drag scale
read from the H--EFTCAMB log for every point. Its lowest sampled ordinate is
$2493.222580514448$ at $\alpha=0.1$, $\Omega_{m0}=0.3075$. The primordial
sector, recombination inputs, calibration, and high-ell nuisance inputs are
held fixed: this records local conditional behavior, not a parameter posterior.

A refinement at $\alpha=0.0975$, $\Omega_{m0}=0.3075$ validates the full KGB
action and the RPH realization at 601 spline nodes. The separate fixed-spectrum
calibration grids give $-2\ln\mathcal L_{\rm Planck}=1063.6712514661913$ for
KGB at $A_{\rm Planck}=1.00275$ and $1060.2194088862268$ for the matched
standard-$\Lambda$CDM spectrum at $A_{\rm Planck}=1.00325$. With the
solver-derived $r_{\rm drag}=147.32\,{\rm Mpc}$, the independently integrated
Pantheon+--DESI--chronometer statistics are $1416.5609849186849$ and
$1422.5765927552$. Thus the executed conditional sums are
...
This comparison matches the matter and radiation densities and all fixed
primordial/recombination inputs, but lets each fixed spectrum take its own
executed $A_{\rm Planck}$ grid minimum. It is therefore neither a common-nuisance
fit nor Bayesian evidence. The KGB point has an independently recomputed
action residual below $2.1\times10^{-81}$ and a local cubic-screening gate
$|\gamma_{\rm PPN}-1|\le1.0463\times10^{-15}$ at $r_V=102.56\,{\rm pc}$.

The same executed KGB point was evolved at 58 transfer redshifts. Direct
integration of the solver's linear total-matter $P(k,z)$ gives $\sigma_8(z)$,
and symmetric $d\sigma_8/d\ln a$ gives $f\sigma_8(z)$ for all 23 rows of the
local RSD compilation. Its largest discrepancy from the independently printed
H--EFTCAMB velocity--density estimator is $9.39\times10^{-5}$; halving the
redshift difference changes the largest prediction by $5.01\times10^{-5}$.
The compilation lacks covariance, window/AP, and nonlinear-likelihood inputs
for its correlated survey groups. It is consequently a native RSD residual
audit and is not added to the conditional sum. The source, immutable records,
and a machine-readable ledger are in \repo.

\section{Reproducibility and claim boundary}

All action-level equations, precision checks, parameter scan, source table, figure, matter prediction, early-universe gate, and PPN screening calculation are regenerated by
\begin{verbatim}
bash scripts/run_all.sh
\end{verbatim}
in \repo. The Planck, late-time, and native-RSD records additionally use the
official Planck distribution, the stated external data files, and the
H--EFTCAMB executable. A statement that R-Universe replaces $\Lambda$CDM
requires a joint parameter posterior, official BAO/supernova/RSD/lensing and
local-gravity likelihoods, and a declared comparison protocol. This technical
note does not predeclare that outcome.
