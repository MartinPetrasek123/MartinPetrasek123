...
To test whether that fixed point is numerically atypical, a deliberately
restricted local grid was then run at
$\alpha\in\{0.030,0.035,0.040,0.045\}$ and
$\Omega_{m0}\in\{0.30825,0.30850,0.30875\}$, holding the primordial sector,
recombination inputs, and initial $A_{\rm Planck}=1$ fixed. At every sampled
$\Omega_{m0}$ an otherwise matched standard-$\Lambda$CDM spectrum was
evaluated by exactly the same official likelihood objects. The KGB local
minimum is $-2\ln\mathcal L=1075.3470998$ at
$\alpha=0.040$, $\Omega_{m0}=0.30850$, while its paired $\Lambda$CDM value is
$1066.1743939$, giving $\Delta(-2\ln\mathcal L)=+9.1727059$. A separate
one-dimensional fixed-spectrum scan of
$A_{\rm Planck}\in\{1.0010,1.0015,1.0020,1.0025,1.0030\}$ has both local
minima at $A_{\rm Planck}=1.0025$: $1066.1369111$ for KGB and $1055.7868759$
for $\Lambda$CDM, hence $\Delta(-2\ln\mathcal L)=+10.3500351$. The
401-, 601-, and 801-node KGB realizations give 1075.3471052, 1075.3470998,
and 1075.3471035, respectively. The profile source and records are in
\repo. This restricted grid is not a parameter posterior, a Bayesian-evidence
calculation, or a complete model comparison. $\Lambda$CDM serves here as an
observational benchmark, while the specified KGB action is the proposed
physical replacement candidate. The reported local profile is higher than
that benchmark and therefore does not yet establish the observational adequacy
required for empirical replacement.
...
\end{verbatim}
in \repo. The Planck fixed-point and conditional-profile evaluations
additionally use the official Planck distribution and the H--EFTCAMB
executable. A statement that R-Universe replaces $\Lambda$CDM requires a joint
parameter posterior, BAO, supernova, RSD, matter-lensing, and local-gravity
inference, followed by a declared model comparison. This technical note does
not predeclare that outcome.
