...
- an exact multi-fluid DAE closure which finds a scalar-constraint singular
  surface on the published reference branch, excluding a global linear
  RFG-R spectrum or likelihood for that branch;
...
- `../manuscript/main.tex` - complete manuscript source and its three
  locally included figures; checked by `validate_manuscript_links.py`.
- `paper/R_Universe_RFG_R_Completion.pdf` - earlier standalone paper build.
...
- `docs/rfg_dae_closure.md` - exact curvature-constraint closure and the
  reproducible singular constraint surface.
- `docs/rfg_xi_completion.md` - a separate background-preserving
  Xi(R3+sigma^2) R-Universe completion and its DAE audit.
...
- `scripts/rfg_dae_closure.py` - analytic multi-fluid DAE reduction and
  constraint-root table.
- `scripts/validate_rfg_dae_closure.py` - independent DAE regression test.
- `scripts/rfg_xi_completion.py` - finite-grid audit of the separate RFG-RXi
  action.
...
extended-EFT scalar degeneracy, runs the exact multi-fluid core audit, and
verifies every manuscript label, citation, included figure, and linked public
calculation path against the checked-in source.
...
constraint reduction and its untruncated kinetic hierarchy are supplied, and
the completed DAE reduction finds `mu_zeta=0` on the reference branch. That
linear singularity prevents a global RFG-R prediction or empirical comparison
with LCDM; a spectrum generated by forcing passage through it would not be a
prediction of the stated action.
