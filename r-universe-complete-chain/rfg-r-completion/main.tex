...
\newcommand{\RFGRepoPath}[1]{\href{\RFGRepoURL/blob/main/r-universe-complete-chain/rfg-r-completion/r\_universe\_completion/#1}{\texttt{#1}}}
\newcommand{\RFGRepoNamed}[2]{\href{\RFGRepoURL/blob/main/r-universe-complete-chain/rfg-r-completion/r\_universe\_completion/#1}{\texttt{#2}}}
...
\large Full proof version: original branch, regular effective completion,\\ local-GR matching, and an exact extended-EFT likelihood boundary}
...
Layer & Status in this manuscript & Precise content \\
\midrule
RFG-R multi-fluid core & Computed action reduction and kinetic hierarchy & Exact Sorkin--Schutz constraint matrix, full \(\bar m_5\) contribution, rational GR rank check, documented RFG-R inertia audit, photon polarization and massless/massive neutrino kinetic equations. \\
Matter/CMB likelihood & Reproducible computation contract; not evaluated & The action derivatives and hierarchy are specified, including nonzero \(\bar m_5\delta{}^{(3)}R\delta K\). An implementation retaining that operator and an actual joint data run remain outstanding. \\
\bottomrule
\end{tabular}%
}
\end{table}
...
The compiled public H-EFTCAMB revision used for a GR reference calculation
exposes the usual \(\Omega,\gamma_1,\ldots,\gamma_6\) interface, but not the
coefficient in Eq.~\eqref{eq:RFGR-m5-operator}.  It therefore cannot produce
an exact RFG-R CMB, lensing, or matter spectrum without an extension of its
scalar equations and stability checks.  Independently of that backend gap,
...
The cosmological background is not a CMB theory. RFG-R supplies the action derivatives, the exact extended-EFT map in Eq.~\eqref{eq:RFGR-extended-EFT-map}, the sourced photon--baryon--CDM--neutrino reduction in Sec.~\ref{sec:rfgr-multifluid-reduction}, parameter set, rejection criteria, likelihood factors, and an exact input/output contract in \RFGRepoNamed{docs/likelihood\_pipeline.md}{likelihood protocol}.  The nonzero operator in Eq.~\eqref{eq:RFGR-m5-operator} is absent from the public stock H-EFTCAMB interface, so that code is a validated GR reference only, not an exact RFG-R Boltzmann implementation. Moreover, the pure-gravity extended-EFT audit in Sec.~\ref{sec:rfgr-extended-scalar-audit} finds a degenerate \(\dot\zeta^2\) coefficient. A complete empirical evaluation must implement the action-defined differential--algebraic hierarchy, retain that operator, and compute recombination, the drag epoch and sound horizon from the same action derivatives. The required extended Boltzmann implementation produces
\[
...
The metric Legendre map is regular on the physical branch, while the full Dirac rank and the cubic scalar cutoff remain open. Exact FLRW has vanishing quadratic scalar kinetic normalization. For the regular RFG-R action, the complete pure-gravity extended-EFT audit likewise yields a degenerate \(\dot\zeta^2\) coefficient despite a nonsingular lapse--shift determinant. The sourced photon--baryon--CDM--neutrino constraint reduction has now been derived from the same action, retaining \(\bar m_5\), and shows one constraint plus four positive finite matter directions; photons and neutrinos remain kinetic hierarchies rather than fluid closures. An action-faithful Einstein--Boltzmann implementation and its Planck/BAO/SN/RSD/GW likelihood have not been run. Thus this paper delivers a falsifiable and reproducible candidate theory with a delimited action-to-solver calculation boundary; it does not claim that RFG-R has already outperformed or replaced \(\Lambda\)CDM.
