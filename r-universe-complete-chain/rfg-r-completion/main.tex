...
\newcommand{\RFGRepoPath}[1]{\href{\RFGRepoURL/tree/main/r-universe-complete-chain/rfg-r-completion/r\_universe\_completion/#1}{\texttt{#1}}}
...
\textbf{RFG-R EFT} & Exact one-field canonical-scalar check; multi-fluid open & \(c_T=1\) on FLRW & Regularized branch & Yes (Eq.~\eqref{eq:RFGR-background}) \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Matter/CMB likelihood & Reproducible computation contract & Action derivatives, stability rejections, required spectra, datasets, and likelihood factors are specified. A compiled RFG-R multi-fluid Boltzmann module and an actual joint data run remain outstanding. \\
\bottomrule
\end{tabular}%
}
\end{table}
...
The generating script is \RFGRepoPath{scripts/generate\_outputs.py}.}
...
\RFGRepoPath{scripts/generate\_outputs.py}.}
...
given in \RFGRepoPath{docs/completion\_derivation.md}. The complete
low-energy EFT is
\begin{equation}
...
script is \RFGRepoPath{scripts/generate\_outputs.py}.}
...
\RFGRepoPath{docs/likelihood\_pipeline.md}. The protocol is compatible with
...
\RFGRepoPath{docs/canonical\_scalar\_reduction.md}; the executable
calculation is \RFGRepoPath{scripts/canonical\_scalar.py} and its machine
readable reference output is
\RFGRepoPath{generated/tables/canonical\_scalar\_reference.csv}.
...
\begin{equation}
 ...
 \min K=1.695411575,\qquad
 \min G_1=0.5673484131,
 \qquad c_{s,\rm low}^2(a=1)=0.3346375721.
...
\RFGRepoPath{README.md}. Its calculation manifest is
\RFGRepoPath{CALCULATION\_MANIFEST.md}; the executable validation command is
...
\RFGRepoPath{docs/canonical\_scalar\_reduction.md},
\RFGRepoPath{scripts/canonical\_scalar.py}, and
...
The present proof version is deliberately conservative. It separates two kinds of closure: a closed mathematical \emph{definition} of an EFT and a completed empirical \emph{test} of that EFT. RFG-R closes the former for its regular cosmological action and its explicitly matched local-GR domain, and it now contains an exact one-field canonical-scalar constraint reduction. It does not convert that restricted diagnostic, an unrun CMB likelihood, or an unresolved multi-fluid matter reduction into a result. A future version may be called a complete fundamental theory only if every remaining gate below is closed by equations and evaluated data likelihoods, not by wording.
...
For the original RFG action, the matter-coupled scalar calculation remains open.  The pure-gravity degeneracy cannot be resolved by importing a result from a different EFT.  For RFG-R, the one-canonical-scalar calculation of Sec.~\ref{sec:rfgr-canonical-scalar} is complete and gives the exact one-dimensional counterpart of the following construction.  The still-open multi-component calculation, for a canonical scalar plus fluids or for the physical photon--baryon--CDM--neutrino system, must take the matrix form
\[
S_{\rm red,m}^{(2)}
=
\frac12
...
\]
This multi-component calculation is not implied by the pure-gravity result because matter sources the lapse and shift constraints.  The one-field RFG-R calculation verifies that the sourced constraints are invertible on a published branch; it neither supplies the multi-fluid kinetic matrix nor settles the original RFG scalar degeneracy.
...
generated tables, validation tests, canonical-scalar reduction, and PPN likelihood factor are publicly
