...
Matter/CMB likelihood & GR data-interface reference executed; RFG-R not evaluated & CAMB 2.0.1 and official Planck low-ell+lensing components run at a fixed GR point. The RFG-R action derivatives and hierarchy include nonzero \(\bar m_5\delta{}^{(3)}R\delta K\); an implementation retaining that operator and an actual joint RFG-R data run remain outstanding. \\
\bottomrule
\end{tabular}%
}
\end{table}
