...
RFG-R\(\Xi\) completion & New action; finite DAE gate passed & The background-null \(\Xi({}^{(3)}R+\sigma_{ij}\sigma^{ij})\) operator is directly varied. At \(\Xi=1\), its 49-by-81 \((a,k)\) audit has no sampled \(\mu_\zeta\) root, but it has no completed Boltzmann or data calculation. \\
Matter/CMB likelihood & GR data-interface reference executed; no R-Universe likelihood & CAMB 2.0.1 and official Planck low-ell+lensing components run at a fixed GR point. The original RFG-R DAE coefficient \(\mu_\zeta\) crosses zero, while RFG-R\(\Xi\) has not been through an action-faithful Boltzmann solver or joint likelihood. \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\RFGRepoNamed{scripts/validate\_rfg\_xi\_completion.py}{its regression test}, and
